...
MEETS = {
    "1 https://meet.google.com/xoq-aooo-nbt": "8:30:30 5/1/2022",
    "2 https://meet.google.com/bsh-qozs-icv": "9:30:30 5/1/2022",
}

DURATION = 60  # Duration of each Meet in minutes
USERNAME = "21148001@pvgcoet.ac.in"
PASSWORD = "Spuk@2003"
BROWSER_DRIVER = "ChromeDrivers/win32/chromedriver.exe"
SERVICEPATH = Service(
    "D:/Skills/Python/Projects/Google Meet Bot/ChromeDrivers/win32/chromedriver.exe"
)
...
def versionCheck():
    ...
    if currentVersionNumber >= latestVersionNumber:
        ...
        print(
            colored(
                "\nGet the latest version at https://github.com/SHUR1K-N/MeetNinja-Google-Meet-Bot",
                "yellow",
            )
        )
        print(
            colored(
                "Every new version comes with fixes, improvements, new features, etc..",
                "yellow",
            )
        )
        print(
            colored(
                "Please do not open an Issue if you see this message and have not yet tried the latest version.",
                "yellow",
            )
        )
...
def initBrowser():
    ...
    if BROWSER_DRIVER.lower().startswith("chrome"):
        ...
        if BROWSER_DRIVER.lower().endswith(".exe"):
            servicePath1 = Service(
                "D:/Skills/Python/Projects/Google Meet Bot/ChromeDrivers/win32/chromedriver.exe"
            )
            servicePath2 = Service("ChromeDriverManager().install()")
        ...
        firefoxOptions.add_argument("--width=800"), firefoxOptions.add_argument(
            "--height=800"
        )
        ...
        if BROWSER_DRIVER.lower().endswith(".exe"):
            driver = webdriver.Firefox(
                executable_path=BROWSER_DRIVER, options=firefoxOptions
            )
        else:
...
def login():
    ...
    usernameNextButton = wait.until(
        when.element_to_be_clickable((by.ID, usernameNextButtonPath))
    )
    usernameNextButton.click()

    passwordField = wait.until(
        when.element_to_be_clickable((by.NAME, passwordFieldPath))
    )
    time.sleep(1)
    passwordField.send_keys(PASSWORD)

    passwordNextButton = wait.until(
        when.element_to_be_clickable((by.ID, passwordNextButtonPath))
    )
...
def attendMeet():
    ...
    try:
        joinButton = wait.until(
            when.element_to_be_clickable((by.XPATH, joinButton1Path))
        )
    except:
        joinButton = wait.until(
            when.element_to_be_clickable((by.XPATH, joinButton2Path))
        )
    if BROWSER_DRIVER.lower().startswith("chrome"):
        time.sleep(1)
    ...
    time.sleep(1)
    print(
        colored(f"Now attending Google Meet #{meetIndex} @{timeStamp()}", "green"),
        end="",
    )
    ...
    try:
        joinButton = wait.until(
            when.element_to_be_clickable((by.XPATH, joinButton1Path))
        )  # For another prompt that pops up for Meets being recorded
        time.sleep(1)
        joinButton.click()
    except:
        pass
...
def genericError():
    ...
    print("\n\nPossible fixes:\n")
    print(
        "1.1 Make sure you have downloaded the latest version of MeetNinja from the GitHub page (every new iteration brings fixes and new capabilities)"
    )
    print(
        "1.2 Make sure you have pip-installed all the required python packages mentioned in the README"
    )
    print(
        "1.3 UNIX-based systems (Linux / Mac): Make sure you have given all the contents of MeetNinja the correct permissions (eg: 'chmod 777 ./ -R')"
    )
    print(
        "2.1 Check your inputs and run MeetNinja again (make sure there are no leading zeros in the Meet start times)"
    )
    print(
        "2.2 And / Or make sure you have chosen the correct webdriver file respective of your web browser and operating system"
    )
    print(
        '3. Make sure the generated web browser is not "Minimized" while MeetNinja is working'
    )
    print(
        "4.1. Make sure the webdriver file is of the latest stable build (https://chromedriver.chromium.org/ or https://github.com/mozilla/geckodriver/releases)"
    )
    print(
        "4.2. And / Or make sure your chosen web browser is updated to the latest version"
    )
    print(
        "4.3. And / Or make sure the webdriver file is at least of the same version as your chosen web browser (or lower)"
    )
    print(
        '5. Make sure the small "time.sleep()" delays (in seconds) in the login() and attendMeet() functions are comfortable for your internet speed'
    )
    ...
    try:
        ...
        for meetIndex, (URL, rawTime) in enumerate(MEETS.items(), start=1):
            ...
            if meetIndex <= 1:
                print(
                    colored(
                        f"Waiting until first Meet start time [{rawTime}]...", "yellow"
                    ),
                    end="",
                )
            else:
